...
1.查看交付物，包含软件源代码光盘
2.通过Console线或SSH终端连接可编程交换机，查看可编程交换机操作系统文件夹，包含软件源代码
1.打开360浏览器/火狐浏览器等国产浏览器
2.输入平台界面管理地址

1.基于前端界面点击系统调度接口
2.点击前端系统调度接口界面显示的接口列表及信息

1.进入可编程交换机交换机命令行基于Crtl+C关闭后端系统;
2.基于Vim修改后端配置代码，添加debug模式启动参数;
3.在可编程交换机命令行基于python deploy Backend.py重启后端;
4.基于前端进入节点端口信息功能界面下发/查询任意信息。



1.采信第三方检验报告
2.通过Console串口线或SSH登录可编程交换机，输入错误用户名与密码
1.查看文档资料交付物是否包含软件设计方案、需求规格说明书、软件设计报告、用户手册、部署手册、安装程序及有关源程序光盘、测试验收文件
...
# 1. 配置信息: "IP地址:目标Label:程序类型(encap/process)"
# 示例：101和102作为中转节点，103作为入口节点
SWITCHES=(
  "192.168.1.101:0x2501:process"
  "192.168.1.102:0x2502:process"
  "192.168.1.103:0x2503:encap"
)

REMOTE_USER="gys"
REMOTE_BASE_DIR="~/Documents/Project/Flep"
# 确保远程环境变量已设置，或者在这里指定绝对路径
SDE_BIN="/opt/sde/install/bin"
SDE_CONF_DEST="/opt/sde/install/share/p4/targets/tofino"

echo "开始执行全流程：修改 Label -> 远程编译 -> 部署配置"
echo "--------------------------------------------------------"
...
for ENTRY in "${SWITCHES[@]}"; do
    ...
    # 根据类型确定目录和文件名
    if [ "$TYPE" == "encap" ]; then
        PROG_NAME="flep_encap"
        DIR_NAME="flep_encap_with_topo"
    else
        PROG_NAME="flep_process"
        DIR_NAME="flep_process_with_topo"
    fi

    echo ">>> [交换机 $IP] 正在处理 $PROG_NAME (Label: $TARGET_LABEL)"

    ssh -t $REMOTE_USER@$IP << EOF
        # 进入工作目录
        cd $REMOTE_BASE_DIR/$DIR_NAME
        
        # --- 1. 修改源码中的 Label ---
        FILE="${PROG_NAME}.p4"
        CURRENT_VALUE=\$(grep "const bit<16> LOCAL_LABEL =" "\$FILE" | grep -oE "0x[0-9a-fA-F]+")
        
        if [ ! -z "\$CURRENT_VALUE" ] && [ "\$CURRENT_VALUE" != "$TARGET_LABEL" ]; then
            echo "   [Update] 修改源码 Label: \$CURRENT_VALUE -> $TARGET_LABEL"
            sed -i "s/const bit<16> LOCAL_LABEL = \$CURRENT_VALUE/const bit<16> LOCAL_LABEL = $TARGET_LABEL/g" "\$FILE"
        fi

        # --- 2. 执行 bf-p4c 编译 ---
        echo "   [Compile] 正在编译 P4 程序..."
        sudo $SDE_BIN/bf-p4c --std p4-16 --target tofino --arch tna \
            -o ./target/ -g ./\$FILE
        
        # --- 3. 分发 .conf 文件 ---
        echo "   [Deploy] 拷贝配置文件到 SDE 目录..."
        sudo cp ./target/${PROG_NAME}.conf $SDE_CONF_DEST/
        
        echo "   [Success] $IP 处理完毕！"
EOF
    echo "--------------------------------------------------------"
done
